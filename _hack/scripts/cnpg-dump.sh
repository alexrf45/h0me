...
SUPERUSER_SECRET="$(kube home -n "$NS" get cluster.postgresql.cnpg.io "$CLUSTER" -o jsonpath='{.spec.superuserSecret.name}')"
PGUSER="$(kube home -n "$NS" get secret "$SUPERUSER_SECRET" -o jsonpath='{.data.username}' | base64 -d)"
PGPASSWORD="$(kube home -n "$NS" get secret "$SUPERUSER_SECRET" -o jsonpath='{.data.password}' | base64 -d)"
export PGPASSWORD
...
kube home -n "$NS" run "cnpg-dump-$STAMP" --rm -i --restart=Never \
